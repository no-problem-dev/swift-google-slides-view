...
@Suite struct GSlidesA2ATests {
    ...
    @Test func streamedPresentationAssemblesAcrossWireRoundTrip() throws {
        let envelope = try GSlidesArtifactCoding.envelopeEvent(
            taskId: "task-1", contextId: "ctx-1", artifactId: "presentation-1",
            presentation: Presentation(title: "My Presentation", slides: [])
        )
        let slide1 = try GSlidesArtifactCoding.slideEvent(
            taskId: "task-1", contextId: "ctx-1", artifactId: "presentation-1", page: slide("s1")
        )
        let slide2 = try GSlidesArtifactCoding.slideEvent(
            taskId: "task-1", contextId: "ctx-1", artifactId: "presentation-1", page: slide("s2"), lastChunk: true
        )
        ...
        #expect(assembler.presentation?.title == "My Presentation")
    ...
    @Test func batchUpdateDiffEventAppliesOverWire() throws {
        // Envelope a presentation with one text element, then stream an element-level edit as a diff event.
        let envelope = try GSlidesArtifactCoding.envelopeEvent(
            taskId: "task-1", contextId: "ctx-1", artifactId: "presentation-1",
            presentation: Presentation(title: "Presentation", slides: [slide("s1")])
        )
    ...
    @Test func secondGSlidesArtifactIsNotMixedIn() throws {
        ...
        let first = try GSlidesArtifactCoding.envelopeEvent(
            taskId: "t", contextId: "c", artifactId: "presentation-1",
            presentation: Presentation(slides: [slide("s1")])
        )
        let other = try GSlidesArtifactCoding.slideEvent(
            taskId: "t", contextId: "c", artifactId: "presentation-2", page: slide("intruder")
        )
    ...
    @Test func metadataVocabularyIsDeclaredOnTheWire() throws {
        let event = try GSlidesArtifactCoding.envelopeEvent(
            taskId: "t", contextId: "c", artifactId: "presentation-1", presentation: Presentation()
        )
